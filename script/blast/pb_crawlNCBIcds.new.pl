...
my $cutoffLength=$pbH{'cutoffLengthFeatures'};  # here the relevant cutoff length is for the feature
...
my $cigiDataType = 'cds'; # this value will now be put into the cigi table do indicate type
...
if ($numGI >= $cutoffNumGINode) # This is a model organism by definition, don't BLAST
	...
	{
	($numCl_node)=blastCluster($gi_node,$cutoffLength,$tiStart,$ti,'node',$cigiDataType,@tiList); 
...
# cluster it anyway. Eventually rewrite numGIsql...
	if ($numGI < $cutoffNumGISub) 
		{
		($numCl_sub)=blastCluster($numGI,$cutoffLength,$tiStart,$ti,'subtree',$cigiDataType,@tiList);
		}
	}
...
print "$sql\n" if $log;
	...
	while ($rowHRef = $sh->fetchrow_hashref)  
			...
			if ($seqLen>0)  # possible that extraction of CDS failed or something else went awry
				{
				print FH ">$gi\n$seq\n";
...
sub  blastCluster
{
	my ($numGI,$cutoffLength,$root_ti,$cur_ti,$cl_type,$cigiDataType,@tiList)=@_;
...
print "Beginning all-all blast ($cl_type): ti$cur_ti ($sciNameH{$cur_ti})\tngi=$numGI\tnti=$numTI\n" if ($log);
	...
	if ($numGI==1) # simple special case of one GI (it happens a lot) .. use values saved at end of fetchSeqs..
		...
		while (<FH>)
			{
			($cl,$gi)=split;
			print FHfcf "$cur_ti\t$cl\t$cl_type\t$cigiDataType\t$gi\t$tiH{$gi}\n"; 
			}
		close FH;
		close FHfcf;
...
for (my $chunk=0; $chunk < $nChunks; $chunk++)
	...
	for my $i (0..$numElem-2)
			{ $queryShort .= " ti=$smallTiList[$i] OR "; }
	$queryShort .= " ti=$smallTiList[$numElem-1])";
	$sql = "select count(*) as ngi from $aaTable,$seqTable where seqs.gi=aas.gi and $queryShort;";
	$sh = $dbh->prepare($sql);
...
sub extract_CDS
		...
		my $dbh = db_connect();
		my $sql="select gi, length_aa, codon_start, range, transl_table from aas where gi_aa=$gi_aa";
		my $sh = $dbh->prepare($sql);
		$sh->execute;
		my ($gi,$length_aa,$codon_start,$range,$transl_table) = $sh->fetchrow_array;  # get the AA stuff

		$sql="select length, seq from seqs where gi=$gi"; # now get the DNA CDS
		$sh = $dbh->prepare($sql);
		$sh->execute;
		my ($length_dna,$seq_dna) = $sh->fetchrow_array;  # only returns one row (presumably) here and next...
		my ($CDS_dna, $CDS_aa) = extract_CDS_bioperl($gi_aa,$range,$codon_start,$transl_table, $seq_dna);
...
if (!defined $transl_table){$transl_table=1}; # enforce default translation table

eval {	

my $nuc_obj = Bio::Seq->new(-display_id => 'temporary',-seq => $seqin, -alphabet => 'dna' );
...
if ($remainder != 0)  # if there is a partial codon at the end of sequence, remove it
	{
	$trimmed_dna_obj = $trimmed_dna_obj->trunc(1,$xlength-$remainder);
	}

$trimmed_aa   = $trimmed_dna_obj->translate(-codontable_id=>$transl_table)->seq;
if ($trimmed_aa =~ /\*$/) # if the last codon is a stop, then delete this codon from dna seq
	{
...
1;
	...
	warn "Bio Perl CDS extraction failed for sequence gi_aa$gi_aa: range $range, frame: $codon_start\n$seqin\n";
