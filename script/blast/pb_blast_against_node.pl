...
if ($gi_query)                       # Query mode
{
    ...
    $sh->execute;
    my $fastaLen = 80;
    if ( my ( $seq, $len ) = $sh->fetchrow_array
...
sub blastCluster {
    ...
    my $maxBlastOut =
      $numGI + 1;    # IMPORTANT TO SET THIS MAX OUTPUT *ABOVE* THE 250 DEFAULT
    $s =
...
# Here just write a cluster table that has a all the hits for a single cluster '0'.
    open FHfcf, ">>$cigiTableFile"
      or print "Couldn't open final cluster file for append";
    open FH, "<$blinkin"
      or print "Couldn't open BLINKIN file $blinkin for re-reading\n";
    $cl = -1
