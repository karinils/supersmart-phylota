...
my $taskId = $ENV{SGE_TASK_ID};    # used to provide unique file names
...
# ***********************
my $cigiDataType =
  'nuc';    # this value will now be put into the cigi table to indicate type
...
# ************************************************************
checkEnvironment()
  ; # check some things on the host compute node to make sure the environment is OK
...
while (<FH>) {
    ...
    if ( $nameClass =~ /scientific name/ ) {
        $sciNameH{$taxid} = $dbh->quote($name);
    } # might as well add these quotes here, cause will use them in the insert command below
...
# Notice the \N is written so mysql will read it as a NULL in its load statement (their convention): i.e., I do not keep anything about cluster numbers
    print FHnodes
"$ti\t$anc\t$terminalNode\t$rankFlag\t$modelFlag\t$sciName\t$comName\t$rankName\t$gi_node\t$gi_sub_nonmodel\t$gi_sub_model\t\\N\t\\N\t\\N\t$nspDesc\t$nspModel\t$n_leaf_desc\t$n_node_desc\t\\N\t\\N\n";
...
sub writeSeqs {
    ...
    open FH, ">$fastaFile"
      or die
"Failed to open fasta file ($fastaFile): maybe target directory was not created on this node ($ENV{HOSTNAME})?\n";
    open FHlen, ">$lengthFile"
      or die "Failed to open length file ($lengthFile)\n";
...
"Beginning all-all blast ($cl_type): ti$cur_ti ($sciNameH{$cur_ti})\tngi=$numGI\tnti=$numTI\n"
    ...
    if ( $numGI == 0 ) {
        close FHfcf;
        return (0);
    }    # NB. I want to open the cluster file above and close it now,
...
#		open FHfcf, ">>$cigiTableFile" or print "Couldn't open final cluster file for append at ti=$cur_ti";
        ...
        while (<FH>) {
            ( $cl, $gi ) = split;
            print FHfcf
              "$cur_ti\t$cl\t$cl_type\t$gi\t$tiH{$gi}\t$cigiDataType\n";
        }
        close FH;
        close FHfcf;
...
# Specificically, tries to use small sample fasta file, $enviro_check_file to format a blast database
    ...
    system($s) == 0
      or die
      "Check of \'$formatdbCom\' FAILED on node $host (error code = $?)\n";
    my $slaveDir = $pbH{'SLAVE_DATA_DIR'};
    if ( !( -e $slaveDir ) ) {
        die
"Check of environment FAILED on node $host: directory $slaveDir does not exist\n";
    }
}
