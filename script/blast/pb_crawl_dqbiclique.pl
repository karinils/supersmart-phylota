...
if ( !( -e $configFile ) ) { die "Missing config file $configFile\n"; }

if ($human) {
    print "Using configuration file $configFile\n";
    print
"Run parameters: mincl=$mincl mintax=$mintax alpha=$alpha PB release=$release\n";
}
...
close FH;
my $dbh = db_connect();
if ( !$magicFile ) { push @magic_tis, $tiStart }
else {
...
for $magic_ti (@magic_tis) {
    ...
    if ( !exists $nodeH{$magic_ti} ) {
        print
"root TI $magic_ti was missing from node hash...probably deleted from NCBI\n";
        next;
    }
...
"Invalid or missing node reference passed to crawlTree...probably deleted TI from NCBI\n"
      if ( !defined $nodeRef );
    if ( 0 == scalar @{ $nodeRef->{DESC} } ) { return }
    ;    # we never build bicliques at terminal nodes
    my $ti = $nodeRef->{ID};
    print "Processing node $ti\n" if $log;
    my $currDQBC = 0;
    my $sql =
"select clustid,ti_of_gi from $clusterTable,$cigiTable where ti_root=$ti and PI=1 and $clusterTable.cl_type='subtree' and ti=ti_root and ci=clustid";
    my $sh = $dbh->prepare($sql);
    $sh->execute;
    my (%h);
    my %clusterSize;

    while ( my ( $clustid, $ti_of_gi ) = $sh->fetchrow_array
    ...
    {
        my @sortedClusters =
          sort { $clusterSize{$b} <=> $clusterSize{$a} } keys %clusterSize;
        ...
        for $ci_complete (@minSortedClusters
          ) # each of these will make a dqbiclique using this cluster as its complete cluster
        {
            my @cia = min_alpha( $ci_complete, \%clusterSize, $alpha,
                @minSortedClusters );
            my @ci_complete_taxa =
              sort { $a <=> $b } keys %{ $h{$ci_complete} };
            ...
            for $ci (@cia_sort) {
                $intersectionRef =
                  intersect( $h{$ci_complete}, $h{$ci} )
                  ;    # takes two hash refs as args
                my @intersectionTax =
                  sort { $a <=> $b } keys %{$intersectionRef};
                my $numIntersectTax = scalar @intersectionTax;
                my $minAlpha        = $alpha * $numCompleteTaxa;
                if (   $numIntersectTax >= $minAlpha
                    && $numIntersectTax >= $mintax )
                {
...
"\nDecisive quasi-biclique at node $ti (max complete taxa = $numCompleteTaxa; num clusters = $numClust; complete ci=$ci_complete)\n\n";
                ...
                for $tx (@ci_complete_taxa) {
                    print "$tx\t\t";
                    for $ci (@ci_keep) {
                        if ( exists $h{$ci}{$tx} ) { ++$countDens; print "X" }
                        else                       { print "." }
                    }
                    print "\n";
                }
        ...
        if ($human) {
            print
"Number of bicliques satisfying specified constraints: $countDQBC\n";
        }
