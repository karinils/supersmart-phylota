...
$blastOutLimit=250;  # number of hits reported in any one blast search!!!! Note impact of this on all-all searches
...
if ($outfileName eq "" || $qfileName eq "" || $tfileName eq "" || $configFile eq ""  )
	{
...
$blastCom =	"$blastDir/bin/blastall" .
		...
		" -p $pbH{'BLAST_PROGRAM'}" .
		" -S $pbH{'BLAST_STRAND'}" .
		" -d $slaveDataDir/blastdb" .   # using a simple name always for the database file
...
sub printUsage
{
die "generic_all_all.pl -q queryfile -t targetfile -o outputfile -c pb config file -l lengthfile\n";
}
