...
while ($fl = shift @ARGV)
  ...
  if ($fl eq '-exclude_sp') {$exclude_sp = 1;} # exclude taxon ids with species names containing ' sp. ' or ' x ' (hybrids)
  ...
  if ($fl eq '-file_by_genus') {$file_by_genus = shift @ARGV;} # print out separate files for each  genus (in each cluster)
  ...
  if ($fl eq '-overlap_file') {$overlap_file = shift @ARGV;} # See how many TIs from this file are present in each cluster
  if ($fl eq '-clusterfile_by_genus') {$clusterfile_by_genus = shift @ARGV;} # print table of taxon names
  if ($fl eq '-clusterfiles') {$clusterfiles = shift @ARGV;} # write a separate fasta file for each cluster with this prefix
  }
...
for $gi  ( @gis )
	...
	$sqls="select ti, $seqField,$lengthField,def from $seqTable where $giField=$gi";
	...
	next if ($exclude_sp && $taxon_name =~ /\sx\s/i);   # contains ' x ' or ' X '; i.e. hybrids
	...
	push @{ $gis_of_genus_ti_allHoA{$ti_genus} }, $gi; # this maintains a list for the whole data set 
...
print "$cl ($count): GIs:$numGIs\tTIs:$numTIs\tGenera:$numGenera (cumul:$cumul_numGenera)\tOV=$countOV\tMin Seq Length:$minL\tMax Seq Length:$maxL\tgi$repGI1\n";
...
print "Total number of clusters overlapping by at least 2 taxa: $countClusters2Plus\n";
print "Total number of TIs participating in these 2+ clusters: $numOVTIsTotal\n";
