...
$nquery    = "\'subtree\'"
  ; # this is required in queries of ci_gi table as long as you are selecting on PI clusters
$fastaLen    = 80;
...
$sql =
"select seqs.gi,seqs.seq from seqs,$cigiTable where $cigiTable.ti=$tiRoot and $cigiTable.clustid=$ci and seqs.gi=$cigiTable.gi and cl_type=$nquery";
$sh = $dbh->prepare($sql);
$sh->execute;
$fastaLen = 80;
