...
# DBOSS
$blastResultsMax = 250
  ; # default used for all-all blast; look out if you have a dense cluster and you bump this up
$trimFlag = 0;    # default
$N_Fraction =
  1.0;    # default is to keep any sequence regardless of how many N's it has
$nquery = 'subtree';
while ( $fl = shift @ARGV ) {
    ...
    if ( $fl eq '-gi' ) {
        $gi_query = shift @ARGV;
    }    # this means we will blast this query gi against the cluster
    if ( $fl eq '-N' ) {
        $N_Fraction = shift @ARGV;
    }   # discard seqs with more than this fraction of N's in the subject length
    if ( $fl eq '-trim' ) {
        $trimFlag = 1;
    }    # report sequences trimmed to their hit boundaries
}
...
open FH2, ">$lengthFile";
$sql =
"select seqs.gi,seqs.seq,seqs.length,seqs.ti from seqs,$clusterTable where $clusterTable.ti=$tiNode and $clusterTable.clustid=$cluster and seqs.gi=$clusterTable.gi and cl_type='$nquery'";
$sh = $dbh->prepare($sql);
$sh->execute;
$fastaLen  = 80;
$countSeqs = 0;
...
if ($gi_query)    # Query mode
{
    ...
    else { die "Query sequence was not found in database\n" }
    $sh->finish;
    close FH1;
}
...
if ($gi_query) {
    $blastResultsMax = $countSeqs + 1;
} # for use in -b option when doing 1-all searches only (keep smaller for all-all)
$blastCom =
...
{
    system
"$scriptDir/blast2blink.mjs.pl -i $outfile -o $outfile\_BLINKIN -t $lengthFile -p $pbH{'OVERLAP_PHI'} -s $pbH{'OVERLAP_SIGMA'} -m $pbH{'OVERLAP_MODE'}\n";
    ...
    while (<FH1>) {
        ...
        $countH{$cl}++;
        $len = length $seqH{$gi};
        if ( !( exists( $minH{$cl} ) ) ) { $minH{$cl} = $len }
        else {
            if ( $len < $minH{$cl} ) { $minH{$cl} = $len }
        }
        if ( !( exists( $maxH{$cl} ) ) ) { $maxH{$cl} = $len }
        else {
            if ( $len > $maxH{$cl} ) { $maxH{$cl} = $len }
        }
    }
    close FH1;
        ...
        # NB! I use minH and maxH differently here...
{
    open FH1, "<$outfile";
    while (<FH1>
      ) # for the one query taxon, find the index of the leftmost starting hit and rightmost ending hit
    {
        (
        ...
        if ( $subjectStart < $minH{$subjectId} ) {
            $minH{$subjectId} = $subjectStart;
        }
        if ( $subjectEnd > $maxH{$subjectId} ) {
            $maxH{$subjectId} = $subjectEnd;
        }
    }
    close FH1;
    $ov = $pbH{OVERLAP_THETA};
    open FH2, ">$outfile\_1QUERY";
    if ($trimFlag) { open FH3, ">$outfile\_1QUERY_TRIM" }
    for $gi ( keys %minH ) {
        ...
        next
          if ( $numNs >= $N_cutoff )
          ;    # skip any sequence entirely if it has too many Ns
        print FH2
"$gi\t$minH{$gi}\t$maxH{$gi}\t$subject_length\t$lengthRatio\t$numNs\n";
