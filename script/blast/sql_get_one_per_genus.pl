...
$sql =
"select taxon_name,seed_gi,length,seq from seqs, $nodeTable, $clusterTable  where seqs.gi=seed_gi and PI=1 and $clusterTable.ti_root=$nodeTable.ti and rank='genus' ";
$sh = $dbh->prepare($sql);
$sh->execute;
$fastaLen = 80;
