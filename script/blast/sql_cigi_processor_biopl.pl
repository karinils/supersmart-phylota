...
# Input: A cigi file from blast search pipeline containing ONE cluster only (assumes this w/o checking)
...
while ( $fl = shift @ARGV ) {
    ...
    if ( $fl eq '-exclude_sp' ) {
        $exclude_sp = 1;
    }    # exclude taxon ids with species names containing ' sp. '
    if ( $fl eq '-one_per_ti' ) {
        $one_per_ti = 1;
...
for $file (@inFiles) {
    ...
    while (<FH>) {
        ...
        $sqls =
          "select $seqField,$lengthField from $seqTable where $giField=$gi";
        $shs = $dbh->prepare($sqls);
        $shs->execute;
        ( $seq, $seqL ) = $shs->fetchrow_array;
    ...
    next if ( $minL < $min_L );
    print
"$file: GIs:$numGIs\tTIs:$numTIs\tGenera:$numGenera (cumul:$cumul_numGenera)\tMin Seq Length:$minL\tMax Seq Length:$maxL\tgi$repGI1\n";
    push @repGI,
...
sub extract_CDS {
    ...
    $sql =
      "select gi, length_aa, codon_start, range from aas where gi_aa=$gi_aa";
    $sh = $dbh->prepare($sql);
    $sh->execute;
    my ( $gi, $length_aa, $codon_start, $range ) =
    ...
    #print "$gi_aa\n";
    my ( $CDS_dna, $CDS_aa ) =
      extract_CDS_bioperl( $range, $codon_start, $seq_dna );
...
sub extract {
    ...
    $s = substr( $seq, $first - 1 + $codon_start - 1, $xlength )
      ;    # start of first codon to end of CDS whether that's codon end or not
    $remainder = $xlength % 3;
    if   ( $remainder == 0 ) { $hanging = 3 }
    else                     { $hanging = $remainder }
    ;      # how many bases are in the last codon in the range?

    if ( $hanging == 3 )    # remove this codon if its a stop
    {
        $last_codon =
          substr( $s, -$hanging );    # get the last $hanging characters
...
sub extract_CDS_bioperl {
 ...
 #print "$trimmed_aa\n";
    ...
    if ( $trimmed_aa =~ /\*/ ) { die "Found a stop codon in the aa sequence\n" }
