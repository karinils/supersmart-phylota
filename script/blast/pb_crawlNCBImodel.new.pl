...
open FH, "<$pbH{'GB_TAXONOMY_DIR'}/names.dmp" or die "No names.dmp file!\n";
while (<FH>) {
...
# put them in one query file and blast them all at once in once invocation of blast
                open FH, ">>$qfastaFile"
                  or warn "Couldn't open qfastaFile: $qfastaFile\n";
                open FHlen, ">>$lengthFile"
                  or warn "Couldn't open lengthFile:$lengthFile\n"
...
sub writeSeqs {
    ...
    my $nSeqs     = 0;
    my $dbh       = db_connect();
    open FH,    ">$fastaFile"  or warn "Couldn't open fastaFile:$fastaFile\n";
    open FHlen, ">$lengthFile" or warn "couldn't open lengthFile:$lengthFile\n";
    ...
    for ( my $chunk = 0 ; $chunk < $nChunks ; $chunk++ ) {
        ...
        while ( $rowHRef = $sh->fetchrow_hashref ) {
            ...
            $seq = $rowHRef->{seq};
            if ( !defined $seq ) {
                print "Warning: no seq for this gi: gi$gi : ti$ti : $sql\n";
            }
            print FH ">$gi\n$seq\n";
            print FHlen "$gi\t$seqLen\n";
        }
...
#system ("$b2bcmd");
    %hitgis = ();
    if ( checkFile($fastaFile) != 1 )  { return \%hitgis }
    if ( checkFile($qfastaFile) != 1 ) { return \%hitgis }
...
sub checkFile {
    my ($f) = @_;
    if ( !( -e $f ) ) { carp "File $f does not exist \n"; return 0; }
    if ( -z $f )      { carp "File $f is empty \n";       return 0; }
    return 1;
}
