...
#print "...done visiting child node " . $descRef->{ID} . "from parent $taxon_name: Checking if child subtree has models...\n";
	...
	if (scalar(@modelTiList)>0 && $this_n_clust_sub>0)  
		...
		open FH, ">>$qfastaFile" or warn "Couldn't open qfastaFile: $qfastaFile\n";
...
for (my $chunk=0; $chunk < $nChunks; $chunk++)
	...
	while ($rowHRef = $sh->fetchrow_hashref)  
			...
			$seq = $rowHRef->{seq};
if (! defined $seq ) {print "Warning: no seq for this gi: gi$gi : ti$ti : $sql\n";}
			print FH ">$gi\n$seq\n";
			print FHlen "$gi\t$seqLen\n";
			}
	}
...
my ($f) = @_;
if (!(-e $f))
	{ carp "File $f does not exist \n" ; return 0; }
if (-z $f)
	{ carp "File $f is empty \n"  ; return 0; }
return 1;
}
