...
die ("$Usage") if (! ($infile && $outfile));
	...
	# ...MJS
	if (-e $infile && -z $infile)  # input exists but has size zero, so there were probably no blast hits all;
		{
		open FH2, ">$outfile";
...
if ($sigmaT || $phiT) {die ("Must supply table if using sigma and phi\n$Usage") if (! $tablefile);}
if (! $sigmaT) {$sigmaT = 0;}
if (! $phiT) {$phiT = 0;}
...
if ($mode)
  ...
  while (<FH3>)
    ...
    if ( $fields[0] =~ /(\w+)/) 
      ...
      else {die ("Problem with format of table file (column 2 is not a number).\n");}
      }
    else {die ("Problem with format of table file (column 0 does not contain seqid).\n");}
    }
  close FH3;
  }
...
# parse blast output -----------------------------------
open FH1, "<$infile" or die "Could not open infile (name=$infile) for reading in blast2blink\n";
open FH2, ">$outfile" or die "Could not open outfile (name=$outfile) for writing in blast2blink\n";
if (! $mode)
  {
  while (<FH1>)
    {
    @d = split;
    ($def1, $def2, $b1, $e1, $b2, $e2) = ($d[0], $d[1], $d[6], $d[7], $d[8], $d[9]);  
    if (($b1>$e1) || ($b2>$e2)) {print "Reversal in hit $def1 $def2\n";}
    $gi1 = $def1;
    $gi2 = $def2;
    if ($targs) {next if ( (! exists $trgGi{$gi1}) || (! exists $trgGi{$gi2}) );}
    print FH2 "$gi1\t$gi2\n";
    }
  }
else  				
  ...
  while (<FH1>)
    ...
    $newGi1 = $def1;
    $newGi2 = $def2;
    if (($newGi1 eq $oldGi1) && ($newGi2 eq $oldGi2)) 		# assumes that all hits between two gis are sequential in blast ouput
      ...
      if ($oldGi1 =~ /\w/)
        ...
        ($sigma1, $sigma2, $phi) =  processRecord ($oldGi1, $oldGi2, $r1ref, $r2ref);
	$accept = acceptRecord ($sigma1, $sigma2, $phi);     
        if ($accept) {print FH2 "$oldGi1\t$oldGi2\n";}
        if ($v) {print "the edge has acceptance:$accept\tbecause:  $sigma1, $sigma2, $phi\n";}
        if ($v) {print "Continue?\n"; $answer = (<STDIN>); die if ($answer =~ /n/i);}
        $oldGi1 = ""; $oldGi2 = "";
	}
      # start new record -------------------------------
      if (  ($targs && (exists $trgGi{$newGi1}) && (exists $trgGi{$newGi2}) ) || (! $targs)  )
        {
        $r1ref = [ [ $b1, $e1 ] ];
        $r2ref = [ [ $b2, $e2 ] ];
        $oldGi1=$newGi1; $oldGi2=$newGi2;
        }
      } # end if new edge
    ...
    if (  ($targs && (exists $trgGi{$newGi1}) && (exists $trgGi{$newGi2}) ) || (! $targs)  )
      {
      if ($v1) {print "calculating stats for last pair, $oldGi1 and $oldGi2:\n";}
      ($sigma1, $sigma2, $phi) =  processRecord ($oldGi1, $oldGi2, $r1ref, $r2ref);
      $accept = acceptRecord ($sigma1, $sigma2, $phi);
      if ($accept) {print FH2 "$oldGi1\t$oldGi2\n";}
      }
...
if ($v1) {print "sigma1: $sigma1 = $sum1/$length1\nsigma2: $sigma2 = $sum2/$length2\n";}
...
if ($v1) {print "phi: $phi = ($hits[1]) / ($lefts[1] + $hits[1] + $rights[1]) \n";} 
return $phi;
}
...
if ($acceptableSum && $acceptablePhi)
  {$pass = 1;}
else {$pass = 0;}
return $pass;
}
