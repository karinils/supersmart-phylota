...
$sql =
"select $seqTable.gi,gi_aa,seq_aa,length_aa from $cigiTable,$seqTable where ti=$tiNode and clustid=$cluster and $seqTable.gi=$cigiTable.gi and cl_type='$nquery' and length_aa>=$lcutoff";
$sh = $dbh->prepare($sql);
$sh->execute;
$fastaLen = 80;
while ( $rowHRef =
