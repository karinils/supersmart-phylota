...
my $ndesc = "";
if ($NO_DESCENT) { $ndesc = "_no_desc" }
;      # munge the file name in this special case
...
#my $sql="select ti,clustid,ti_of_gi from $clusterTable,$cigiTable where ti_root=$ti and PI=1 and $clusterTable.cl_type='$nQuery' and ti=ti_root and ci=clustid";
    my $sql =
"select ti,clustid,ti_of_gi from $clusterTable,$cigiTable where ti_root=$ti and PI=1 and $cigiTable.cl_type='$nQuery' and ti=ti_root and ci=clustid";
    my $sh = $dbh->prepare($sql);
    $sh->execute;
    my $clustersPresent = 0;
    while ( my ( $ti, $clustid, $ti_of_gi ) = $sh->fetchrow_array
