...
my $sql="select ti,clustid,ti_of_gi from $clusterTable,$cigiTable where ti_root=$tiNode and PI=1 and $clusterTable.cl_type='$nQuery' and ti=ti_root and ci=clustid";

my $sh = $dbh->prepare($sql);
$sh->execute;
while (($ti,$clustid,$ti_of_gi) = $sh->fetchrow_array)  # only returns one row (presumably) here and next...
	{
