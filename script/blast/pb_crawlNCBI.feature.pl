...
if ($cigiDataType eq 'ourRNA') # handles a special 'or' query for multiple RNA types...
	{$cigiQuery = "(primary_tag='misc_RNA' OR primary_tag='rRNA')";}	
else
	{$cigiQuery = "primary_tag='$cigiDataType'";}	
...
my 	$nodeTableFile = "$slaveWorkingDir/rti$tiStart.nodes_feat.id$taskId"; # this will store node table entries for this run
...
my ($fake_gi,$gi,$gi_aa,$length_aa,$ti,$seq,$numTI,$query,$queryShort,$sql,$sh,$rowHRef,$numGI,$numGIShort);
$numTI=@tiList;
if ($numTI == 0)
...
for (my $chunk=0; $chunk < $nChunks; $chunk++)
	...
	$sql = "select gi,ti,feature_id,length,seq from $featureTable where $cigiQuery and $queryShort;";
print "$sql\n" if $log;
	$sh = $dbh->prepare($sql);
...
if ($nSeqs==1)
	{$saveTI=$ti;$saveGI = $fake_gi;}  # hack to save some crap below for frequent case of one gi in a taxon
else
	{$saveTI=-1;$saveGI = -1;}  
return $nSeqs;
}
...
sub  blastCluster
{
	...
	my ($seqLen,$ti,$numGIcl,$numTI,$numCl,$sql,$sh,$rowHRef,$gi,$fake_gi,$def,$seq,$cl,$i);
	my ($clusterID,$s);
...
print "Beginning all-all blast ($cl_type): ti$cur_ti ($sciNameH{$cur_ti})\tngi=$numGI\tnti=$numTI\n" if ($log);
	...
	if ($numGI==1) # simple special case of one GI (it happens a lot) .. use values saved at end of fetchSeqs..
		...
		if ( system ($s) != 0 ) 
			...
			print "$s\n";
			print "...while attempting to cluster: blast ($cl_type): ti$cur_ti ($sciNameH{$cur_ti})\tngi=$numGI\tnti=$numTI\n" ;
	        	if ($? == -1) {
			...
			print "Copying any intermediate results files back to head node...\n";
my $s= "cp $cigiTableFile $headWorkingDir\n";
system $s;
$s= "cp $nodeTableFile $headWorkingDir\n";
system $s;
		...
		while (<FH>)
			{
			($cl,$fake_gi)=split;
			print FHfcf "$cur_ti\t$cl\t$cl_type\t$fake_gi\t$tiH{$fake_gi}\t$cigiDataType\n"; 
			}
		close FH;
		close FHfcf;
...
for (my $chunk=0; $chunk < $nChunks; $chunk++)
	...
	for my $i (0..$numElem-2)
			{ $queryShort .= " ti=$smallTiList[$i] OR "; }
	$queryShort .= " ti=$smallTiList[$numElem-1])";
	$sql = "select count(*) as ngi from $featureTable where $cigiQuery and $queryShort;";
	$sh = $dbh->prepare($sql);
