...
# ----- read genbank flatfile, select records to include, make new files --------------------------
        ...
        while (<FH1>) {
            ...
            ++$records;
            if ( $record =~
/LOCUS\s+\w+\s+(\d+)\sbp.{4}(\w{1,6}).+([A-Z]{3})\s+(\d+)\-([A-Z]{3})\-(\d{4})/
              )    # GenBank record respects column positions ...
            {
                $length   = $1;
                $molType  = $2;
                $division = $3;
                ...
                if ( $length <= $sizeCriterion ) {
                    ( $gid, $tid, $taxon, $def, $keywords, $defline, $sequence )
                      = ( &extractGnb($record) );
                    $quotedDef = $dbh->quote($def)
                      ; # DBI function takes care of problem with imbedded quotes in INSERT below
                    ++$acceptedRecords;
                    $acceptedBases += $length;
                    $s =
"INSERT INTO $seqTable VALUES($gid,$tid,$length,\'$division\',\'$fmt_date\',$release,\'$molType\',$quotedDef,\'$sequence\')";
                    $dbh->do("$s");
                }
                else  # sequence is LARGE ,don't retrieve the sequence proper...
                {
                    ( $gid, $tid, $taxon, $def, $keywords, $defline ) =
                      &extractGnbNoSeq($record);
                    $quotedDef = $dbh->quote($def)
                      ; # DBI function takes care of problem with imbedded quotes in INSERT below
                    ++$largeSeqs;
                    $s =
"INSERT INTO $seqTable VALUES($gid,$tid,$length,\'$division\',\'$fmt_date\',$release,\'$molType\',$quotedDef,NULL)";
                    $dbh->do("$s");
...
# Notice I do not use the defline that Shelley constructed here. I do use the DEFINITION from GB
{
    ...
    ($org) = ( $gnb =~ /ORGANISM\s+(.+)/ );
    if ( !( ($ti) = ( $gnb =~ /db_xref=["']taxon:?(\d+)["']/ ) ) ) { $ti = 0; }

    #($seq) = ($gnb =~ /ORIGIN[^\n]*\n(.+)/s);
    ($seq) = ( $gnb =~ /^ORIGIN[^\n]*\n(.+)$/ms )
      ; # MJS: my fix for the rare case where word ORIGIN is stuck in title, etc.
    chomp $seq;
    $seq =~ s/\d//g;
    $seq =~ s/\s//g;
...
# note to self: can change the delim to % but dare not mess with comment on the first line or the switches on the last!
    ...
    if ( $numCDS > 0 ) {
        for ( $ix = 0 ; $ix < $numCDS ; $ix++ ) {
            ...
            $trans    = $cds[ $ix * 2 + 1 ];
            $trans =~ s/\s//g;
            $length_aa = length($trans)
              ;    # make sure to count aas before adding apostrophes!
            $trans = "\'$trans\'";
            if ( ($range) =
                ( $cds_misc =~ m%^\s+(\w*\(*[<>,\.\d\s]+\)*)%s )
              )    # only took 2 hours to get his right
## ARGH still not right, does not handle 'complement(join(1..3,5..6...
            ...
            if   ( $cds_misc =~ m%/db_xref="GI:(\d+)"% ) { $gi_aa = $1 }
            else                                         { $gi_aa = 'NULL' }
            if ( $cds_misc =~ m%/transl_table=(\d+)% ) { $transl_table = $1 }
            else { $transl_table = 'NULL' }
            if ( $cds_misc =~ m%/codon_start=(\d+)% ) { $codon_start = $1 }
            else                                      { $codon_start = 'NULL' }
            if   ( $cds_misc =~ m%/gene="(\w+)"% ) { $gene = "\'$1\'" }
            else                                   { $gene = 'NULL' }
...
#print "$gi\t$gi_aa\t$transl_table\t$codon_start\t$gene\t$range\n$cds_misc\n$trans\n";
            ++$count_cds;
            $s =
"INSERT INTO $aaTable VALUES($gi,$gi_aa,$length_aa,$codon_start,$transl_table,$gene,$range,$trans)";
