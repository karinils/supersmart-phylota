...
$sql =
"select seqs.gi,seqs.seq,seqs.def from seqs,$clusterTable where $clusterTable.ti=$tiNode and $clusterTable.clustid=$cluster and seqs.gi=$clusterTable.gi and cl_type='$nquery'";
$sh = $dbh->prepare($sql);
$sh->execute;
$fastaLen = 80;
