...
$sql =
"select ci,ti_root,ti_anc,ci_anc,n_gi from $clusterTable,$nodeTable where ti_root=ti and PI=1 and ci_anc IS NOT NULL limit 100 ";
print "$sql\n";
$sh = $dbh->prepare($sql);
$sh->execute;
...
#    this is determinative for all children
    $sqls =
"select n_gi,supernum from $clusterTable where ci=$ci_anc and ti_root=$ti_anc and cl_type='subtree'";
    $shs = $dbh->prepare($sqls);
    $shs->execute;
    if ( $Hs = $shs->fetchrow_hashref
      )    # only returns one row (presumably) here and next...
    {
        ...
        if ( $ngi == $ngi_anc ) {
            ++$matchCount;
            $s =
"update $clusterTable set supernum=1 where ti_root=$ti_anc and ci=$ci_anc and cl_type='subtree'";
        }
        else {
            ++$nomatchCount;
            $s =
"update $clusterTable set supernum=0 where ti_root=$ti_anc and ci=$ci_anc and cl_type='subtree'";
