...
my $sql =
"select ti,taxon_name,n_otu_desc from $nodeTable where terminal_flag=0 and rank=\"\'family\'\" and n_sp_desc>$curCutoff";
...
$sh->finish;
$sql =
"select ti,n_otu_desc from $nodeTablePrev where terminal_flag=0 and rank=\"\'family\'\"";
$sh = $dbh->prepare($sql);
$sh->execute;
while ( $H = $sh->fetchrow_hashref ) {
