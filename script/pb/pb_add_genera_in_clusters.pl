...
while (($ti,$clustid,$cl_type,$ti_of_gi) = $sh->fetchrow_array)  
	...
	next if (!$genus); # sometimes there is no genus for a sequence: e.g., undescribed taxon of uncertain rank
	$H{$ky}{$genus}++;
	}
$sh->finish;
...
#	print "$ky\t$numGenera\t";
	...
	$s = "UPDATE $clusterTable set n_gen=$numGenera where ti_root=$ti and ci=$clustid and cl_type='$cl_type'";
	++$numUpdates;
	$dbh->do($s);
	}
