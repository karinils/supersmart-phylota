...
while ( $fl = shift @ARGV ) {
    ...
    if ( $fl =~ /-magic/ ) {
        $magicFile = $par;
    } # file with the root tis of clades that were computed separately on the cluster
}
die if ( !($configFile) || !( -e $magicFile ) );
...
sub recurseSum {
    ...
    my $H;
    if (
        exists $tiReturnNode{$ti}
      )    # just hit a node that is the root of a tree with data
    {
        my $sql = "select * from $nodeTable where ti=$ti";
        my $sh  = $dbh->prepare($sql);
        $sh->execute;
        if ( $H = $sh->fetchrow_hashref ) {
            return ( $H->{n_gi_node}, $H->{n_gi_sub_nonmodel},
                $H->{n_gi_sub_model}, $H->{n_sp_desc}, $H->{n_leaf_desc},
                $H->{n_otu_desc} );
        }
        else {
            print
"Node $ti is apparently not present in the phylota database, causing an error here \n";
            return ( 0, 0, 0, 0, 0, 0 );
        }
    }
    else # presumably this is a node toward the root of the whole tree, not yet having reached a subtree with data
    {
        ...
        my ( $terminalNode, $rank, $anc, $rankFlag, $comName, $sciName,
            $rankName );
####### Get information about this node #####
        if   ( 0 == $numDesc ) { $terminalNode = 1; }
        else                   { $terminalNode = 0; }
        $rank = $rankH{$ti};
 ...
 # following determines the six basic fields regarding the gi tallies for a node
		...
		$n_leaf_desc,
		$n_otu_desc,
		NULL,	# two fields added later in development
		NULL
		)";

        #print "$s\n";
        $dbh->do("$s");
        return ( $n_gi_node, $n_gi_sub_nonmodel, $n_gi_sub_model, $n_sp_desc,
            $n_leaf_desc, $n_otu_desc );
    }
}
