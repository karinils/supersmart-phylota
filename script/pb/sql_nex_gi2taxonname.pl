...
while (<>)
	...
	if ($matrixFlag)
		...
		if (/;/) 
			{ $matrixFlag = 0 ; last; }
		++$countMatrixLines;
		if (/(\'.*?\')/) # non greedy match to see if there is a label in single quotes
			{
			$label = $1; # if so, remove qoutes, hyphens, and replace whitespace with underscores (hack--not fully compliant with nexus conventions)
...
#			print "$label  => .........\n";
			if ($label =~ /ti\d+/)
				{  push @labelsWithoutGiWithTi, $label;  }
			else
				{  push @labelsWithoutGiWithoutTi, $label;  }
			$seqH{$label}=$seq;
			}
		}
	else
		{print;}	# for anything not in the matrix just print as is
	}
print "matrix\n";
print "[Taxa with gis:",scalar @newLabelsWithGis,"]\n";
for $label (@newLabelsWithGis)
	{ print "$label\t$seqH{$label}\n"  }
print "[Taxa with no gis, but tis:",scalar @labelsWithoutGiWithTi,"]\n";

for $label (@labelsWithoutGiWithTi)
	{ print "$label\t$seqH{$label}\n"  }
print "[Taxa with no gis, no tis:",scalar @labelsWithoutGiWithoutTi,"]\n";
for $label (@labelsWithoutGiWithoutTi)
	{ print "$label\t$seqH{$label}\n"  }
print "\n;\nEND;\n";
...

{
	...
	$sqls="select gi,gi_feat,$nodeTable.ti,taxon_name from $nodeTable,features where $nodeTable.ti=features.ti and gi_feat=$gi";
	$shs = $dbh->prepare($sqls);
	$shs->execute;
	my ($gi_s, $gi_feat,$ti1,$name1) = $shs->fetchrow_array;

	$sqls="select gi, $nodeTable.ti,taxon_name from $nodeTable,seqs where $nodeTable.ti=seqs.ti and gi=$gi";
	$shs = $dbh->prepare($sqls);
	$shs->execute;
	my ($gi_seq,$ti2,$name2) = $shs->fetchrow_array;
	$shs->finish;

	if (defined $gi_feat && defined $gi_seq) { die "Conflict between seq and feature gis (gi:$gi) in lookup\n"; }

	if (! (defined $gi_seq) && ! (defined $gi_s))  # last ditch...add one to accession number version and try again
		{
