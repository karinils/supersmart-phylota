...
$dbh->do("$s");
$s =
"INSERT INTO $clusterTable select $cigiTable.ti,clustid,cl_type, feature_type, count($cigiTable.gi),count(distinct $seqsTable.ti), count(distinct $seqsTable.ti)>=4, min(length),max(length), sum(length)/(max(length)*count($cigiTable.gi)),NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL from $seqsTable,$cigiTable where feature_type='nuc' and $cigiTable.gi=$seqsTable.gi group by $cigiTable.ti,clustid,cl_type,feature_type";
print "$s\n";
$dbh->do("$s");
$s =
"INSERT INTO $clusterTable select $cigiTable.ti,clustid,cl_type, feature_type, count($cigiTable.gi),count(distinct $featuresTable.ti), count(distinct $featuresTable.ti)>=4, min(length),max(length), sum(length)/(max(length)*count($cigiTable.gi)),NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL from $featuresTable,$cigiTable where feature_type='cds' and $cigiTable.gi=$featuresTable.feature_id group by $cigiTable.ti,clustid,cl_type,feature_type";
print "$s\n";
$dbh->do("$s");
$s =
...
$sql =
"select ti_root, ci,cl_type, feature_type from $clusterTable where feature_type='nuc'";
$sh = $dbh->prepare($sql);
$sh->execute;
while ( $H = $sh->fetchrow_hashref ) {
    ...
    $sqls =
"select $seqsTable.gi from $seqsTable,$cigiTable where $cigiTable.ti=$ti and $cigiTable.clustid=$clustid and $cigiTable.cl_type=$cl_type and $cigiTable.feature_type=$feature_type and $seqsTable.gi=$cigiTable.gi order by length desc limit 1";
      ...
      )    # only returns one row (presumably) here and next...
    {
        $gi = $Hs->{gi};
        $s =
"update $clusterTable set seed_gi=$gi where ti_root=$ti and ci=$clustid and cl_type=$cl_type and feature_type=$feature_type";
...
$count = 1;
$sql =
"select ti, ti_anc, model,ci,cl_type,feature_type,seed_gi from $nodeTable,$clusterTable where $nodeTable.ti=$clusterTable.ti_root and feature_type='nuc';";
...
# notice here to get the ancestor, we'll want to look in the subtree clusters always
    $sqls =
"select clustid from $cigiTable where ti=$ti_anc and cl_type=\'subtree\' and feature_type=$feature_type and gi=$seed_gi";
      ...
      )    # only returns one row (presumably) here and next...
    {
        $ci_anc = $Hs->{clustid};
        $s =
"update $clusterTable set ci_anc=$ci_anc where ti_root=$ti and ci=$ci and cl_type=$cl_type and feature_type=$feature_type";
