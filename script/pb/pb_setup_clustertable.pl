...
$s = "INSERT INTO $clusterTable select $cigiTable.ti,clustid,cl_type, count($cigiTable.gi),count(distinct $seqsTable.ti), count(distinct $seqsTable.ti)>=4, min(length),max(length), sum(length)/(max(length)*count($cigiTable.gi)),NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL from $seqsTable,$cigiTable where $cigiTable.gi=$seqsTable.gi group by $cigiTable.ti,clustid,cl_type";
#print "$s\n";
...
# BUG FOUND 4/3/2012. I neglected to add cl_type key to this select
	$sqls = "select $seqsTable.gi from $seqsTable,$cigiTable where $cigiTable.ti=$ti and $cigiTable.clustid=$clustid and $cigiTable.cl_type=\'$cl_type\' and $seqsTable.gi=$cigiTable.gi order by length desc limit 1";
