...
while ($rowHRef = $sh->fetchrow_hashref)  # only returns one row (presumably) here and next...
	...
	if ($clustCount > 0)
		...
		if ($atLeastOneMatch) 
			{
			print "NAME FOUND (WITH NO CLUSTER) AND CONGENER CLUSTER FOUND (#congeners = $numCong)\n";
			++$numTaxaHittingCongener;
			}
		else 
			{
			print "NAME FOUND (WITH NO CLUSTER) AND NO CONGENER CLUSTER FOUND\n";
			}
		 }			
	}
$sh->finish;
...
if ($rowCount == 0)
	...
	if ($atLeastOneMatch) 
		{
		print "NAME NOT FOUND BUT CONGENER CLUSTER FOUND (#congeners = $numCong)\n";
		++$numTaxaHittingCongener;
		}
	else
		{
...
print "Num input taxa with exact match to name having a cluster: $numExactMatch\n";
print "Num input taxa with match to at least one congener having a cluster: $numTaxaHittingCongener\n";
...
for $ti (keys %h)
	...
	if ($name =~ /(\w+_\w+)_\w+/)  # trinomial
		...
		if ($nH{$1})
			{ 
			print "Matching binomial $1 found for trinomial $name (Removing it!)\n";
			delete $seqH{$ti};
			} 
		else
			{ print "No matching binomial $1 found for trinomial $name\n" }
		}
	}
}
...
# they are congeners. Just do the bookkeeping to track these. Adds these to the global hashes.

		{
			...
			my $sql="select rank,taxon_name,ti from $tablename where taxon_name LIKE \"$genus%\" order by taxon_name";
			my $sh = $dbh->prepare($sql);
			$sh->execute;
			my $numCongeners = 0;
			$atLeastOneMatch = 0;
			while ($rowHRef = $sh->fetchrow_hashref)  # loop over all leaves of this genus
				{
				$rank =$rowHRef->{rank};
...
#				print "Genus $genus\tCongener: $tn (ti$ti)\n";
				if ($seqH{$ti} ) 
					{ ++$numCongeners; $atLeastOneMatch=1; next; } # needn't consider this further; it's already been seen
				($clustCount,$tn,$longestSeq)=clusterSearch($ti,$searchString);
				if ($clustCount>0)
					{
					$nameH{$ti}=$tn;
			...
			if ($atLeastOneMatch == 0)
				{ push @noCongenerClusterList, $qname };
...
my $sql = "select taxon_name,length,gi,seq,ci,cl_type, def from $tablename,$clusterTable,seqs where seqs.ti=$tablename.ti and ti_root=$ti and seed_gi=gi and cl_type='node' and def REGEXP \"$searchString\" order by length desc limit 1";
my $rowCount;
my $sh = $dbh->prepare($sql);
$sh->execute;
while ($rowHRef = $sh->fetchrow_hashref)  
