...
# DO NOT USE feat->length for split sequences at all! unless you want the length of the whole region from min to max
                foreach my $feat ( $seqobj->get_SeqFeatures() ) {
                    if ( $feat->primary_tag =~ /CDS|RNA/ ) {
                        my (
                            $tag,      $range,        $trans,
                            $gene,     $transl_table, $codon_start,
                            $gi_feat,  $protein_id,   $acc,
                            $acc_vers, $product
                        );
...
# otherwise it takes care of any imbedded quotes
                        ...
                        $s =
"INSERT INTO $featureTable VALUES(0,$primary_tag,$gi,$gi_feat,$ti,$acc,$acc_vers,$feature_length,$codon_start,$transl_table,$gene,$product,$range,$feature_sequence)"
                          ; # the first 0 is for the autoincrement to work right...thought I could omit it
                            #print "$s\n";
              ...
              or do {    # caught an error in the eval for this sequence
                warn
"My code caught a Bioperl or DBI error parsing the 'next sequence'\n";
                print
"The mysql string was (which may or may not be relevant!):\n$s\n";
