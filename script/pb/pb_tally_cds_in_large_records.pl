...
$time0         = time();
$sizeCriterion = 25000
  ; # If sequence length is strictly greater than this, store information but not the sequence string itself
if ( $sizeCriterion > 65535 ) {
    $sizeCriterion = 65535;
}    # this is the max size of mysql TEXT object
while ( $fl = shift @ARGV ) {
    $par = shift @ARGV;
...
/LOCUS\s+\w+\s+(\d+)\sbp.{4}(\w{1,6}).+([A-Z]{3})\s+(\d+)\-([A-Z]{3})\-(\d{4})/
          )    # GenBank record respects column positions ...
        {
            ...
            if ( $length > $sizeCriterion && $length < 250000 ) {
                ...
                if ( $numCDS > 0 ) { print "$file:$gi:\t$length\t$numCDS\n"; }
                $totalCDS += $numCDS;
            }
        }
    }
