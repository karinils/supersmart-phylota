...
$alignments_done = 0;
$trees_done      = 0
  ; # initially we'll record that these are not done yet at the time this script is run
while ( $fl = shift @ARGV ) {
    $par = shift @ARGV;
...
$sh->finish;
$sh = $dbh->prepare(
    "select count(*) from $nodeTable where n_gi_node>0 || n_gi_sub_nonmodel>0");
$sh->execute;
while ( $rowHRef =
    $sh->fetchrow_hashref ) # only returns one row (presumably) here and next...
{
    $nodesWithSequence = $rowHRef->{'count(*)'};
}    # nodes with short sequence at node proper or short nonmodel seq at subtree
$sh->finish;
...
$sh->finish;
$s =
"insert into $tableName values($gb_rel,\'$gb_rel_date\',$numGIs,$numNodes,$numTerms,$nodesWithClusts,$nodesWithClustsSub,$nodesWithSequence,$n_clusts,$n_PI_clusts,$n_singleton_clusts,$n_large_gi_clusts,$n_large_ti_clusts,$n_largest_gi_clust,$n_largest_ti_clust,$alignments_done,$trees_done)";
print "$s\n";
$dbh->do("$s");
