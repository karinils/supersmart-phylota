...
$seqio_obj = Bio::SeqIO->new(-file => $faFile, -format => "nexus" );
...

{
	...
	if ($gi_type == 1)
		{ $sqls="select $nodeTable.ti,taxon_name from $nodeTable,seqs,aas where $nodeTable.ti=seqs.ti and seqs.gi=aas.gi and gi_aa=$gi";}
	else
		{ $sqls="select $nodeTable.ti,taxon_name from $nodeTable,seqs where $nodeTable.ti=seqs.ti  and $gi_type =$gi";}
