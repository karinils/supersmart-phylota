...
-- a single sequence can have multiple features associated with it. we are
-- probably most interested in the ones that are cds/exon features?
...
create table if not exists features(
		...
		gi bigint(20) unsigned,  -- sequence id
        index(gi),
        -- foreign key(gi) references seqs(gi), -- references gi in seqs table
        
		gi_feat bigint unsigned,
        index(gi_feat),
        
		ti bigint(20) unsigned default null, -- taxon ID, why not go through seqs?
        index(ti),
        -- foreign key(ti) references nodes(ti), -- references ti in nodes table
        
		acc varchar(12), -- genbank accession number
        acc_vers smallint unsigned, -- genbank accession version
        length bigint unsigned, -- feature length
